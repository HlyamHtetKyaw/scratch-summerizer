    ...
    intervalRef.current = window.setInterval(async () => {
      try {
        ...
        if (result.status === 'COMPLETED') {
          setData(result.transcript ?? "No content returned");
          setStatus('COMPLETED');
        ...
        // Optional: Implement max retries here to avoid infinite loops on network down
      }
...
const AudioUploader = ({ userId = 1 }: { userId?: number }) => {
  ...
  const handleUpload = async () => {
    ...
    formData.append("type", taskType); // Matches @RequestParam("type")
    formData.append("userId", userId.toString());
    
    // Only send style if summarizing, otherwise send NONE or let backend default handle it
  ...
  return (
    <div className="p-6 border rounded-lg shadow-lg max-w-lg mx-auto bg-white">
      ...
      <div className="flex gap-4 mb-4">
        ...
        {/* Show Style Dropdown only if Summarize is selected */}
        {taskType === 'SUMMARIZE' && (
            <div className="flex-1">
      ...
      {data && status === 'COMPLETED' && (
        <div className="transcript-box mt-4 p-4 bg-gray-50 border rounded-lg">
          <div className="flex justify-between items-center mb-2">
            <h3 className="font-bold text-gray-700">Result</h3>
            <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">Completed</span>
          </div>
          <p className="whitespace-pre-wrap text-gray-700 leading-relaxed">{data}</p>
        </div>
      )}
    </div>
  );
